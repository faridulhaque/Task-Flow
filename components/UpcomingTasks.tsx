    ...
    <div className="w-11/12 max-w-md bg-gray-700 py-10 px-6 mx-auto mt-10 rounded-lg shadow-xl space-y-6">
      ...
      {data?.map((d: TaskPayload) => {
        return (
          <div
            key={d?._id}
            className={`w-full bg-gray-700 border  shadow-md rounded-md mt-4 p-4 flex items-center justify-between hover:border-indigo-500 transition-all duration-200 relative ${"border-blue-500"}`}
          >
            <div className="flex items-start space-x-4">
              {/* <div className="mt-1 w-6 h-6 border-2 border-gray-400 rounded-sm flex items-center justify-center cursor-pointer hover:border-indigo-500 transition"></div> */}
